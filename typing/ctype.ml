  ...
  try
    ...
    | _ ->
        ...
        with Not_found ->
          ...
          | (Timplicit_arrow (id1, t1, u1, _),
             Timplicit_arrow (id2, t2, u2, _)) ->
              moregen inst_nongen type_pairs subst env t1 t2;
          ...
          | (Tpackage (p1, n1, tl1), Tpackage (p2, n2, tl2)) ->
              begin try
                unify_package env
                  (moregen_list inst_nongen type_pairs subst env)
                  t1'.level p1 n1 tl1 t2'.level p2 n2 tl2
      ...
      | Reither(c1, tl1, _, e1), Reither(c2, tl2, m2, e2) ->
          if e1 != e2 then begin
            ...
            if List.length tl1 = List.length tl2 then
              ...
              t2 :: _ ->
                List.iter
                  (fun t1 -> moregen inst_nongen type_pairs subst env t1 t2)
                  tl1
            | [] ->
...
let moregeneral env inst_nongen pat_sch subj_sch =
  ...
  *)
  ...
  let res =
    try
      moregen inst_nongen (TypePairs.create 13) Subst.identity env patt subj;
      true
    with Unify _ ->
      false
  in
    ...
    | _ ->
        ...
        with Not_found ->
          ...
          | (Tunivar _, Tunivar _) ->
              unify_univar t1' t2' !univar_pairs
          | Tconstr (p1, tl1, _), Tconstr (p2, tl2, _)
            when (not (Path.is_application p1)) &&
                 (not (Path.is_application p2)) &&
                 Ident.Map.mem (Path.head p1) !equality_equations &&
                 Ident.Map.mem (Path.head p2) !equality_equations ->
              let (tl, tr) =
                (* Base-kinded have priority *)
                if (tl1 = [] && tl2 <> []) then
                  (t1', t2')
                else if (tl2 = [] && tl1 <> []) then
                  (t2', t1')
                else if p1 < p2 (* FIXME: ?? *) then
                  (t1', t2')
                else
                  (t2', t1')
              in
          ...
          | Tconstr (p, _, _), _
            when (not (Path.is_application p)) &&
                 Ident.Map.mem (Path.head p) !equality_equations ->
...
and eqtype_modulo_equation rename type_pairs subst env lhs rhs =
  ...
  | Tconstr (path, [], _) ->
    let equations =
      try Ident.Map.find (Path.head path) !equality_equations
