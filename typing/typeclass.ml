...
let enter_value_val_and_par label unbound_reason val_env par_env =
  let (id, val_env) = Env.enter_unbound_value label unbound_reason val_env in
  let par_env = Env.add_unbound_value id unbound_reason par_env in
  id, val_env, par_env

let add_value_val_and_par id val_env par_env =
  Env.add_unbound_value id Val_unbound_self val_env,
  Env.add_unbound_value id Val_unbound_self par_env
  ...
  let already_declared, id, val_env, par_env =
    ...
    | None, _ | _, true ->
        let id, val_env, par_env =
          enter_value_val_and_par label Val_unbound_instance_variable
            val_env par_env
        in
           ...
           let (val_env, par_env, super) =
             ...
             | Some {txt=name} ->
                 let (_id, val_env, par_env) =
                   enter_value_val_and_par name Val_unbound_ancestor
                     val_env par_env
                 in
...
and class_structure cl_num virt self_scope final val_env met_env loc
  ...
  let val_env, par_env =
    List.fold_right
      (fun {pv_id; _} (val_env, par_env) ->
         add_value_val_and_par pv_id val_env par_env)
  ...
  let ty =
    match final with
    | Final -> Ctype.newobj (Ctype.newvar())
    | Not_final -> self_type
  in
