...
let add_unbound_val_and_par id unbound_reason val_env par_env =
  Env.add_unbound_value id unbound_reason val_env,
  Env.add_unbound_value id unbound_reason par_env

let enter_ancestor_val_and_par label val_env par_env =
  let id = Ident.create_local label in
  add_unbound_val_and_par id Val_unbound_ancestor val_env par_env

let add_self_val_and_par id val_env par_env =
  add_unbound_val_and_par id Val_unbound_self val_env par_env

let add_instance_var_val_and_par id val_env par_env =
  add_unbound_val_and_par id Val_unbound_instance_variable val_env par_env

let add_instance_vars_val_and_par vars val_env par_env =
  List.fold_left
    (fun (val_env, par_env) (_, id) ->
       add_instance_var_val_and_par id val_env par_env)
  ...
  | Pcf_inherit (override, sparent, super) ->
      Builtin_attributes.warning_scope cf.pcf_attributes
        (fun () ->
           ...
           in
           let val_env, par_env =
             add_instance_vars_val_and_par inherited_vars val_env par_env
           in
           let vars =
             List.fold_left
           ...
           let (val_env, par_env, super) =
             ...
             | Some {txt=name} ->
                 let (val_env, par_env) =
                   enter_ancestor_val_and_par name val_env par_env
                 in
  ...
  let val_env, par_env =
    List.fold_right
      (fun {pv_id; _} (val_env, par_env) ->
         add_self_val_and_par pv_id val_env par_env)
