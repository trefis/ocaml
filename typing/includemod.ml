...
and try_modtypes ~loc env ~mark cxt subst mty1 mty2 =
  ...
  | (Mty_functor(param1, Some arg1, res1),
     Mty_functor(param2, Some arg2, res2)) ->
      ...
      in
      let cc_res =
        modtypes ~loc (Env.add_module param1 Mta_present arg2' env) ~mark
...
and signature_components ~loc old_env ~mark env cxt subst paired =
  ...
  | (Sig_module(id1, pres1, mty1, _),
     Sig_module(_id2, pres2, mty2, _), pos) :: rem -> begin
      ...
      | Mta_present, Mta_present, _ -> (pos, cc) :: rem
      | _, Mta_absent, _ -> rem
      | Mta_absent, Mta_present, Mty_alias p1 ->
          (pos, Tcoerce_alias (env, p1, cc)) :: rem
      | Mta_absent, Mta_present, _ -> assert false
    end
...
let path_of_context = function
    Module id :: rem ->
      let rec subm path = function
          [] -> path
        | Module id :: rem -> subm (Pdot (path, Ident.name id)) rem
        | _ -> assert false
