...
module Simple_head_pat : sig
  ...
  let of_pattern q =
    let loc = q.pat_loc in
    let typ = q.pat_type in
    let env = q.pat_env in
    let attributes = q.pat_attributes in
    ...
    in
    { loc; typ; env; attributes;
      desc = get_desc q.pat_desc; }
