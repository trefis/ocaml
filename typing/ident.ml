...
let rec find_stamp s = function
    None ->
      raise Not_found
  | Some k ->
      if (binding_time k.ident)= s then k.data else find_stamp s k.previous
...
let rec find_same id = function
  ...
  | Node(l, k, r, _) ->
      let c = compare (name id) (name k.ident) in
      if c = 0 then
        let id_stamp = binding_time id in
        if id_stamp = (binding_time k.ident)
        then k.data
...
let hash i = (Char.code (name i).[0]) lxor (binding_time i)
