  ...
  let ids_decls =
    List.map
      (fun name ->
        ...
        let (id, new_env) =
          (* TODO: Ensure that long path is not ensured. *)
            ...
            | None ->
                ...
                | Tobject (fields, _) ->
                    ...
                    let collect_fields li (meth, meth_kind, _meth_ty) =
                      if field_kind_repr meth_kind =
                         Fpublic then meth::li else li in
...
and type_label_access env srecord usage lid =
  ...
  in
  try suspended ()
  with (Env.Error.In_context _ | Error.In_context _) when !Clflags.typing_recovery ->
...
and type_function_cases_expect
  ...
  Builtin_attributes.warning_scope attrs begin fun () ->
    ...
    if !Clflags.typing_recovery then
      try delayed () with
      | _ ->
          let () = Typing_recovery.erroneous_type_register ty_expected in
          cases, partial, ty_fun
    else delayed ()
  end
