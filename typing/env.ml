...
module TycompTbl =
  struct
    ...
    type 'a t = {
      current: ('a * address option) Ident.tbl;
    ...
    and 'a opened = {
      components: (string, ('a * address option) list) Tbl.t;
    ...
    let rec find_all name tbl =
      List.map (fun (_id, (desc, _)) -> desc, nothing)
      ...
      | Some {using; next; components} ->
          ...
          | opened ->
              List.map
                (fun (desc, _) -> desc, mk_callback rest name desc using)
                opened
              @ rest
...
module IdTbl =
  struct
    ...
    and 'a opened = {
      ...
      components: (string, 'a * address option) Tbl.t;
    ...
    let rec find_name ~mark name tbl =
      try
        let (id, (desc, addr)) = Ident.find_name name tbl.current in
        Pident id, desc, addr
      with Not_found as exn ->
        begin match tbl.opened with
        | Some {using; root; next; components} ->
            begin try
              let descr, addr = Tbl.find_str name components in
              let res = Pdot (root, name), descr, addr in
              if mark then begin match using with
              | None -> ()
              | Some f -> begin
                  match find_name ~mark:false name next with
                  | exception Not_found -> f name None
                  | _, descr', _ -> f name (Some (descr', descr))
                end
              end;
              res
            with Not_found ->
    ...
    let rec update name f tbl =
      try
        ...
        {tbl with current = Ident.add id (new_desc, addr) tbl.current}
      with Not_found ->
        begin match tbl.opened with
        | Some {root; using; next; components} ->
            begin try
              let (desc, addr) = Tbl.find_str name components in
              let new_desc = f desc in
              let components = Tbl.add name (new_desc, addr) components in
    ...
    let rec find_all name tbl =
      List.map
        (fun (id, (desc, _)) -> Pident id, desc)
        (Ident.find_all name tbl.current) @
      match tbl.opened with
      | None -> []
      | Some {root; using = _; next; components} ->
          try
            let (desc, _) = Tbl.find_str name components in
    ...
    let rec iter f tbl =
      ...
      | Some {root; using = _; next; components} ->
          Tbl.iter
            (fun s (x, _) -> f (Ident.hide (Ident.create s) (* ??? *)) (Pdot (root, s), x))
            components;
          iter f next
      | None -> ()
...
and module_components =
  {
    ...
    loc: Location.t;
    comps:
      (t * Subst.t * Path.t * address option * Types.module_type,
...
and 'a comp_tbl = (string, ('a * address option)) Tbl.t
and 'a tycomp_tbl = (string, ('a * address option) list) Tbl.t

and structure_components = {
  mutable comp_values: value_description comp_tbl;
  mutable comp_constrs: constructor_description tycomp_tbl;
  mutable comp_labels: label_description tycomp_tbl;
...
let components_of_module_maker' =
  ref ((fun (_env, _sub, _path, _addr, _mty) -> assert false) :
          t * Subst.t * Path.t * address option * module_type ->
          module_components_repr option)
...
let rec find_module_descr path env =
  ...
  | Pdot(p, s) ->
      begin match get_components (find_module_descr p env) with
        Structure_comps c ->
          fst (Tbl.find_str s c.comp_components)
      | Functor_comps _ ->
         raise Not_found
      end
  | Papply(p1, p2) ->
...
let find proj1 proj2 path env =
  match path with
    Pident id ->
      fst (IdTbl.find_same id (proj1 env))
  | Pdot(p, s) ->
      begin match get_components (find_module_descr p env) with
        Structure_comps c ->
          let (data, _addr) = Tbl.find_str s (proj2 c) in data
      | Functor_comps _ ->
          raise Not_found
      end
  | Papply _ ->
...
let find_module_with_addr ~alias path env =
  match path with
    Pident id ->
      begin try
        let data, addr = IdTbl.find_same id env.modules in
        EnvLazy.force subst_modtype_maker data, addr
      with Not_found ->
        if Ident.persistent id && not (Ident.name id = !current_unit) then
          let ps = find_pers_struct (Ident.name id) in
          md (Mty_signature(Lazy.force ps.ps_sig)), Some (Aident id)
        else raise Not_found
      end
  | Pdot(p, s) ->
      begin match get_components (find_module_descr p env) with
        Structure_comps c ->
          let (data, addr) = Tbl.find_str s c.comp_modules in
          EnvLazy.force subst_modtype_maker data, addr
      | Functor_comps _ ->
          raise Not_found
      end
...
let find_idtbl_address_opt proj1 proj2 path env =
  ...
  | Pdot(p, s) -> begin
      ...
      | Structure_comps c -> snd (Tbl.find_str s (proj2 c))
...
let find_tycomptbl_address_opt proj1 proj2 path env =
  ...
  | Pdot(p, s) -> begin
      match get_components (find_module_descr p env) with
      | Structure_comps c ->
          let comps = Tbl.find_str s (proj2 c) in
          List.fold_left
            (fun acc (_, addr) ->
              match addr with
              | None -> acc
...
let rec lookup_module_descr_aux ?loc ~mark lid env =
  ...
  | Ldot(l, s) ->
      ...
      begin match get_components descr with
        Structure_comps c ->
          let (descr, _addr) = Tbl.find_str s c.comp_components in
          (Pdot(p, s), descr)
      | Functor_comps _ ->
          raise Not_found
      end
...
let lookup proj1 proj2 ?loc ~mark lid env =
  ...
  | Ldot(l, s) ->
      ...
      begin match get_components desc with
        Structure_comps c ->
          let (data, _addr) = Tbl.find_str s (proj2 c) in
          (Pdot(path, s), data)
      | Functor_comps _ ->
          raise Not_found
      end
...
let find_all_comps proj s (p,mcomps) =
  match get_components mcomps with
    Functor_comps _ -> []
  | Structure_comps comps ->
      try let (c,_) = Tbl.find_str s (proj comps) in [Pdot(p,s), c]
      with Not_found -> []
...
let add_to_tbl id decl addr tbl =
  ...
  Tbl.add id ((decl, addr) :: decls) tbl
      ...
      List.iter2 (fun item path ->
        match item with
          Sig_value(id, decl) ->
            ...
            in
            c.comp_values <-
              Tbl.add (Ident.name id) (decl', addr) c.comp_values;
        | Sig_type(id, decl, _) ->
        ...
        | Sig_module(id, pres, md, _) ->
            ...
            in
            c.comp_modules <-
              Tbl.add (Ident.name id) (md', addr) c.comp_modules;
            let deprecated =
            ...
            in
            c.comp_components <-
              Tbl.add (Ident.name id) (comps, addr) c.comp_components;
            env := store_module ~check:false id pres md !env
        | Sig_modtype(id, decl) ->
            let decl' = Subst.modtype_declaration sub decl in
            c.comp_modtypes <-
              Tbl.add (Ident.name id) (decl', None) c.comp_modtypes;
            env := store_modtype id decl !env
        | Sig_class(id, decl, _) ->
            let decl' = Subst.class_declaration sub decl in
            c.comp_classes <-
              Tbl.add (Ident.name id) (decl', next_address ()) c.comp_classes
        | Sig_class_type(id, decl, _) ->
            let decl' = Subst.cltype_declaration sub decl in
            c.comp_cltypes <-
              Tbl.add (Ident.name id) (decl', None) c.comp_cltypes)
        sg pl;
...
and add_module_declaration ?(arg=false) ~check id presence md env =
  let env = store_module ~check id presence md env in
...
and enter_module_declaration ?arg id presence md env =
...
let enter_module ?arg s presence mty env =
  let id = Ident.create s in
  (id, enter_module_declaration ?arg id presence (md mty) env)
