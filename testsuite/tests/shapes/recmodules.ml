...
[%%expect{|
{("A", module) -> .11: {("t", type) -> .11: A/302 . "t"[type];
                        };
 ("B", module) -> .12: {("t", type) -> .12: B/303 . "t"[type];
                        };
 }
module rec A : sig type t = Leaf of B.t end
and B : sig type t = int end
...
{("A", module) -> {("compare", value) -> .38: ·;
 ...
 ("ASet", module) ->
     {("compare", value) ->
          CU Stdlib . "Set"[module] . "Make"[module](.19: A/324) .
          "compare"[value];
      ("elt", type) ->
          CU Stdlib . "Set"[module] . "Make"[module](.19: A/324) .
          "elt"[type];
      ("t", type) ->
          CU Stdlib . "Set"[module] . "Make"[module](.19: A/324) . "t"[type];
      };
 }
module rec A :
  sig
