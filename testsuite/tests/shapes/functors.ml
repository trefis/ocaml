...
module Falias (X : S) = X
[%%expect{|
{("Falias", module) -> .4: Abs(X/279, .3: X/279);
 }
...
{("Finclude", module) ->
     .6:
     Abs(X/283,
         {("t", type) -> .5: X/283 . "t"[type];
          ("x", value) -> .5: X/283 . "x"[value];
          });
 }
...
{("Big_to_small1", module) ->
     .40:
     Abs(shape-var/386,
         <internal>: {("t", type) -> <internal>: shape-var/386 . "t"[type];
                      });
 }
...
{("Big_to_small2", module) ->
     .42:
     Abs(shape-var/392,
         {("t", type) -> .41: <internal>: shape-var/392 . "t"[type];
          });
 }
