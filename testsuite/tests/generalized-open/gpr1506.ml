...
1 | include struct open struct type t = T end let x = T end
                   ^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: The type t/328 introduced by this open appears in the signature
       Line 1, characters 46-47:
         The value x has no valid type if t/328 is hidden
|}];;
...
Error: The type t/334 introduced by this open appears in the signature
       Line 7, characters 8-9:
         The value y has no valid type if t/334 is hidden
|}];;
