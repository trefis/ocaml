...
<<<<<<< HEAD:lambda/matching.ml
    | ([], _) :: _ -> assert false
    | [ ((p, ps, _) as cl) ]
      when rev_yes <> [] && List.for_all omega_like (p:: ps) ->
||||||| parent of 667fb6af7... Add intermediary types to keep track of the compilation state of pms:bytecomp/matching.ml
    | ([], _) :: _ -> assert false
    | [ ((p, ps, _) as cl) ]
      when rev_yes <> [] && List.for_all heads_are_var (p:: ps) ->
=======
    | [ (((p, ps), _) as cl) ]
      when rev_yes <> [] && group_var p && List.for_all heads_are_var ps ->
>>>>>>> 667fb6af7... Add intermediary types to keep track of the compilation state of pms:bytecomp/matching.ml
    ...
    | (((p, _), _) as cl) :: rem ->
        if can_group group_discr p && safe_before Simple.to_pattern cl rev_no then
