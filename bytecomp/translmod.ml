...
let init_shape modl =
  ...
  and init_shape_struct env sg =
    ...
    | Sig_typext _ :: _ ->
        raise Not_found
    | Sig_module(id, Mta_present, md, _) :: rem ->
        init_shape_mod env md.md_type ::
        init_shape_struct (Env.add_module_declaration ~check:false
                             id Mta_present md env) rem
    | Sig_module(id, Mta_absent, md, _) :: rem ->
        init_shape_struct
          (Env.add_module_declaration ~check:false
                             id Mta_absent md env) rem
...
let rec bound_value_identifiers = function
  ...
  | Sig_module(id, Mta_present, _, _) :: rem ->
      id :: bound_value_identifiers rem
...
and transl_structure loc fields cc rootpath final_env = function
  ...
  | item :: rem ->
      ...
      | Tstr_exception ext ->
          ...
          size
      | Tstr_module ({mb_presence=Mta_present} as mb) ->
          let id = mb.mb_id in
          ...
          Llet(pure_module mb.mb_expr, Pgenval, id,
               module_body,
               body), size
      | Tstr_module {mb_presence=Mta_absent} ->
          transl_structure loc fields cc rootpath final_env rem
...
let rec defined_idents = function
  ...
  | item :: rem ->
    ...
    | Tstr_module {mb_id; mb_presence=Mta_present} -> mb_id :: defined_idents rem
    | Tstr_module {mb_presence=Mta_absent} -> defined_idents rem
...
let rec more_idents = function
  ...
  | item :: rem ->
    ...
    | Tstr_module
        {mb_presence=Mta_present; mb_expr={mod_desc = Tmod_structure str}}
    | Tstr_module
        {mb_presence=Mta_present;
         mb_expr={mod_desc=
...
and all_idents = function
  ...
  | item :: rem ->
    ...
    | Tstr_module
        {mb_id;mb_presence=Mta_present;mb_expr={mod_desc = Tmod_structure str}}
    | Tstr_module
        {mb_id;mb_presence=Mta_present;
         mb_expr=
           {mod_desc =
              Tmod_constraint ({mod_desc = Tmod_structure str}, _, _, _)}} ->
        mb_id :: all_idents str.str_items @ all_idents rem
    | Tstr_module {mb_id;mb_presence=Mta_present} -> mb_id :: all_idents rem
    | Tstr_module {mb_presence=Mta_absent} -> all_idents rem
...
let transl_store_structure glob map prims aliases str =
    ...
    | item :: rem ->
        ...
        | Tstr_value(rec_flag, pat_expr_list) ->
            ...
            Lsequence(Lambda.subst subst lam,
                      transl_store rootpath (add_idents false ids subst) cont rem)
        | Tstr_primitive descr ->
            record_primitive descr.val_val;
            transl_store rootpath subst cont rem
        | Tstr_type _ ->
        ...
        | Tstr_module{mb_id=id;mb_loc=loc;mb_presence=Mta_present;
                      ...
                      Llet(Strict, Pgenval, id,
                           ...
                           Lsequence(store_ident loc id,
                                     transl_store rootpath
                                                  ...
                                                  cont rem)))
        | Tstr_module{
            mb_id=id;mb_loc=loc;mb_presence=Mta_present;
            mb_expr= {
                      ...
                      Llet(Strict, Pgenval, id,
                           ...
                           Lsequence(store_ident loc id,
                                     transl_store rootpath
                                                  ...
                                                  cont rem)))
        | Tstr_module
            {mb_id=id; mb_presence=Mta_present; mb_expr=modl;
  ...
  and store_alias (pos, env, path, cc) cont =
...
let transl_store_gen module_name ({ str_items = str }, restr) topl =
  ...
  let f = function
    ...
    | str -> transl_store_structure module_id map prims aliases str in
...
let transl_toplevel_item item =
  ...
  | Tstr_module {mb_id=id; mb_presence=Mta_present; mb_expr=modl} ->
