...
let rec build_class_init cla cstr super inh_init cl_init msubst top cl =
  ...
  | Tcl_constraint (cl, _, vals, meths, concr_meths) ->
      ...
      | Tcl_ident (path, _, _), (obj_init, path', path_lam)::inh_init ->
...
let transl_class ids cl_id pub_meths cl vflag =
  ...
  and linh_envs =
    List.map
      (fun (_, _, path_lam) -> Lprim(Pfield 3, [path_lam], Location.none))
      (List.rev inh_init)
  in
  let make_envs lam =
  ...
  let inh_paths =
    List.filter
      (fun (_,path,_) -> List.mem (Path.head path) new_ids) inh_init
  in
  let inh_keys =
    List.map
      (fun (_, _, path_lam) -> Lprim(Pfield 1, [path_lam], Location.none))
      inh_paths
  in
  let lclass lam =
