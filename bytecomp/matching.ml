...
(** A default environment (referred to as "reachable trap handlers" in the
    paper), is an ordered list of [matrix * raise_num] pairs, and are used to
      ...
      | _ -> (
          ...
          | _ ->
              ...
              let rebuild_default nexts def =
                (* Assuming we had bound [top_default] from the result of
                   [split_or], then we could have written this as:
                   {[
                     List.map
                       (fun (mat, e) -> add_omega_column mat, e)
                       top_default
                   ]}
                   But that would actually be doing more work: it would add an
                   omega column to all the matrices in top_default, whereas here
                   we're reusing the matrices that were in [def], and only
                   adding the omega column to the new matrices.
                *)
                List.fold_right
                  (fun (e, pmh) ->
