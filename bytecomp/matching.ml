...
let half_simplify_cases args cls =
  let arg =
    ...
    | (arg, _) :: _ -> arg
  in
  let rec half_simplify_ = function
    | [] -> []
    | ([], _) :: _ -> assert false
    | ((pat :: patl, action) as cl) :: rem -> (
        match pat.pat_desc with
        | Tpat_any -> cl :: half_simplify_ rem
        | Tpat_var (id, s) ->
            let p = { pat with pat_desc = Tpat_alias (omega, id, s) } in
            half_simplify_ ((p :: patl, action) :: rem)
        | Tpat_alias (p, id, _) ->
        ...
        | Tpat_record ([], _) -> (omega :: patl, action) :: half_simplify_ rem
        | Tpat_record (lbls, closed) ->
            let all_lbls = all_record_args lbls in
            let full_pat =
              { pat with pat_desc = Tpat_record (all_lbls, closed) }
            in
            (full_pat :: patl, action) :: half_simplify_ rem
        | Tpat_or _ -> (
            let pat_simple = half_simplify_or pat in
            match pat_simple.pat_desc with
            | Tpat_or _ -> (pat_simple :: patl, action) :: half_simplify_ rem
            | _ -> half_simplify_ ((pat_simple :: patl, action) :: rem)
          )
        | _ -> cl :: half_simplify_ rem
      )
  in
  half_simplify_ cls
