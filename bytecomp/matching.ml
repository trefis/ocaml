...
let same_group discr p =
  let group = get_group discr in
  group p
...
and split_no_or cls args def k =
  (* [collect_group_raising] tries to raise all the rows
     of the currently-collected group that can be placed
     before the following rows.

     [collect_group_splitting] is a more naive approach where
     we never assume that a compatible row can be raised above
     a non-compatible row, and split eagerly. We use it for
     extension constructors, that in the common case cannot
     be permuted above each other due to constructor rebinding
     (they may be equal). Using [collect_group_raising] would
     be correct, but would have quadratic complexity
     -- the worst case.*)
  let rec split cls =
    ...
    match discr.pat_desc with
      ...
      | Tpat_construct (_, { cstr_tag = Cstr_extension _ }, _) ->
         collect_group_splitting discr [] cls
      | _ ->
         collect_group_raising discr [] [] cls
  and collect_group_raising discr rev_yes rev_no = function
    | ([], _) :: _ -> assert false
    | ((p :: _, _) as cl) :: rem ->
        if same_group discr p && safe_before cl rev_no then
          collect_group_raising discr (cl :: rev_yes) rev_no rem
        else
          collect_group_raising discr rev_yes (cl :: rev_no) rem
    | [] -> (
  ...
  and collect_group_splitting discr rev_yes = function
    | ([], _) :: _ -> assert false
    | ((p :: _, _) as cl) :: rem ->
        if same_group discr p then
          collect_group_splitting discr (cl :: rev_yes) rem
        else
          insert_split (List.rev rev_yes) cl rem
    | [] ->
  ...
  and insert_split yes cl rem =
    ...
    | _ ->
        ...
        ( { me = Pm { cases = yes; args; default = def };
          ...
          (idef, next) :: nexts )
  and final_group group =
    ( { me = Pm { cases = group; args; default = def };
        matrix = as_matrix group;
        top_default = def
      },
      k )
  and final_vars group =
    precompile_var args group def k
  in
  split cls
